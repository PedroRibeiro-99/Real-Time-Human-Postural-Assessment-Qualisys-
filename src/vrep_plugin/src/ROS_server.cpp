	...
	for (int pubI=0;pubI<int(publishers.size());pubI++)
	{
		...
		{ // we handle only the publishers that are not asleep!
			...
			if (publishers[pubI].cmdID==simros_strmcmd_receive_data_from_script_function)
			{
				...
				if (simCallScriptFunctionEx(options,publishers[pubI].auxStr.c_str(),stackHandle)!=-1)
				{ // success!
					...
					if (D.readDataFromStack_scriptFunctionCall(stackHandle,outArgs,outArgs[0],functionName.c_str()))
					{
						...
						fl.intData.data.assign(outData->at(0).int32Data.begin(),outData->at(0).int32Data.end());
...
bool ROS_server::simRosCallScriptFunctionService(vrep_common::simRosCallScriptFunction::Request &req,vrep_common::simRosCallScriptFunction::Response &res)
{
	...
	CScriptFunctionData D;
	D.pushOutData_scriptFunctionCall(CScriptFunctionDataItem(req.inputInts));
	D.pushOutData_scriptFunctionCall(CScriptFunctionDataItem(req.inputFloats));
	D.pushOutData_scriptFunctionCall(CScriptFunctionDataItem(req.inputStrings));
	D.pushOutData_scriptFunctionCall(CScriptFunctionDataItem(req.inputBuffer.c_str(),req.inputBuffer.size()));
    int stackHandle=simCreateStack();
	D.writeDataToStack_scriptFunctionCall(stackHandle);
	res.result=-1;
	...
	if (simCallScriptFunctionEx(req.scriptHandleOrType,req.functionNameAtObjectName.c_str(),stackHandle)!=-1)
	{ // success!
		// Now check the return arguments:
        const int outArgs[]={4,sim_script_arg_int32|sim_script_arg_table,0,sim_script_arg_float|sim_script_arg_table,0,sim_script_arg_string|sim_script_arg_table,0,sim_script_arg_charbuff,0};
		if (D.readDataFromStack_scriptFunctionCall(stackHandle,outArgs,outArgs[0],functionName.c_str()))
		{
			res.result=0;
			std::vector<CScriptFunctionDataItem>* outData=D.getOutDataPtr_scriptFunctionCall();
			res.outputInts.assign(outData->at(0).int32Data.begin(),outData->at(0).int32Data.end());
